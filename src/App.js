      ...
      <div className="container max-w-lg m-auto p-8 pt-16 flex">
        <div className="flex-1">
          <input
            type="button"
            value={
              sessionAddress ? `Logout ${address?.slice(0, 5)}...` : "Login"
            }
            onClick={(e) =>
              (sessionAddress ? logout() : login()).catch((err) => alert(err))
            }
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault()
              signAndSendAction("createThread", inputRef.current.value).catch(
                (err) => alert(err)
              )
          ...
          <div>
            ...
            {views.get("/threads")?.map((row, index) => (
              <div
                key={index}
                className="p-4 px-5 rounded-lg border-2 border-gray-500 mb-4 break-words"
              >
                <div>{row.title}</div>
